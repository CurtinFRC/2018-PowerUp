...
void PathfinderMPMode::init() {
  _ecfg = {
    _enc->GetEncoder(),
    _cfg.enc_ticks_per_rev, _cfg.wheel_diameter * 0.0254 * PI, _cfg.kp, _cfg.ki, _cfg.kd, _cfg.kv, _cfg.ka
  };
  _follow = { 0, 0, 0, 0, 0 };
}

double PathfinderMPMode::calculate() {
  double ret = pf_follow_enc(_ecfg, &_follow, _segments, _seg_length, _enc->GetEncoder());
